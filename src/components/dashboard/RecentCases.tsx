  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200">
      <div className="flex items-center justify-between p-6 border-b border-gray-100">
        ...
        <Link to="/cases" className="text-sm text-accent-500 hover:text-accent-600 font-medium flex items-center">
          View all <ChevronRight className="w-4 h-4 ml-1" />
        </Link>
      </div>
      <div className="divide-y divide-gray-50">
        {cases.length === 0 ? (
          <div className="p-6 text-center text-gray-400 text-sm">No cases found</div>
        ) : (
          cases.map((c) => (
            <Link key={c.id} to={`/cases/${c.id}`} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
              <div className="min-w-0">
                ...
                <div className="text-sm font-medium text-gray-900 mt-0.5 truncate">{c.title}</div>
                <div className="text-xs text-gray-500">{c.clientName}</div>
              </div>
              <div className="text-xs text-gray-400 text-right flex-shrink-0 ml-4">{c.lastActivity}</div>
            </Link>
          ))
        )}
      </div>
